...
pub fn node_to_category(
    ...
    if is_named {
        ...
        if kind == "property_identifier" && text == "constructor" && parent_kind == Some("method_definition") {
        ...
        (ScopeCategory::Property, "punctuation.separator.colon.property".into())
    } else if text == "${" {
...
fn named_node(
    ...
    match kind {
        ...
        "string_fragment" => {
            (ScopeCategory::String, "source.ts string.quoted.template".into())
        }
        "template_string" => {
            (ScopeCategory::String, "source.ts string.quoted.template".into())
        }
        ...
        "hash_bang_line" => {
            (ScopeCategory::Comment, "source.ts comment.line.hashbang".into())
        }

        "property_identifier" => match (parent_kind, grandparent_kind) {
            (Some("method_definition"), _) => {
                (ScopeCategory::Function, "source.ts entity.name.function".into())
            }
            (Some("member_expression"), Some("call_expression")) => {
                (ScopeCategory::Function, "source.ts meta.function-call.property".into())
            }
            _ => {
                (ScopeCategory::Property, "source.ts variable.other.property".into())
            }
        },
        "private_property_identifier" => {
            (ScopeCategory::Property, "source.ts variable.other.property.private".into())
        }

        "identifier" => match (parent_kind, field_name) {
            (Some("function_declaration" | "generator_function_declaration"
                  | "method_definition" | "method_signature"
                  | "abstract_method_signature" | "function_expression"
                  | "generator_function"), Some("name")) => {
                (ScopeCategory::Function, "source.ts entity.name.function".into())
            }
            (Some("call_expression"), Some("function")) => {
                (ScopeCategory::Function, "source.ts meta.function-call".into())
            }
            ...
            | (Some("assignment_expression"), Some("left")) => {
                (ScopeCategory::Variable, "source.ts variable.other.readwrite".into())
            }
            (Some("formal_parameter" | "required_parameter"
                   | "optional_parameter"), _) => {
                (ScopeCategory::Parameter, "source.ts variable.parameter".into())
            }
            ...
            (Some("type_annotation" | "type_alias_declaration" | "type_parameter"
                   | "type_arguments"), _) => {
        ...
        "this" | "super" => {
            (ScopeCategory::Keyword, "source.ts keyword.this".into())
        }
...
fn anonymous_token(text: &str) -> (ScopeCategory, String) {
    match text {
        ...
        | "abstract" | "async" | "await" | "yield" | "new"
        | "typeof" | "instanceof" | "keyof" | "readonly" | "static"
        | "public" | "private" | "protected" | "declare" | "as"
        | "satisfies" | "in" | "of" | "using" | "with" | "module"
        | "namespace" | "from" | "require" | "import" | "export"
        | "get" | "set" => {
        ...
        "`" => {
            (ScopeCategory::String, "source.ts string.quoted.template.backtick".into())
        }
...
mod tests {
    ...
    fn test_function_name() {
        let (cat, _) =
            named_node("identifier", Some("function_declaration"), None, Some("name"));
    ...
    fn test_variable_name() {
        let (cat, _) =
            named_node("identifier", Some("variable_declarator"), None, Some("name"));
    ...
    fn test_call_property() {
        let (cat, _) = named_node("property_identifier", Some("member_expression"), Some("call_expression"), None);
        assert_eq!(cat, ScopeCategory::Function);
    }
    ...
    fn test_class_name() {
        let (cat, _) =
            named_node("identifier", Some("class_declaration"), None, Some("name"));
