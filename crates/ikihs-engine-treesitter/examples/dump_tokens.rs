...
fn main() {
    ...
    let expected_str = Command::new("python3").args(["-c", &format!(
        r#"
import json
with open('{}') as f:
    data = json.load(f)
...
for i, color in enumerate(c):
    if color:
        print(f'{{{{i}}}}{{{{color}}}}')
"#, dir.join("javascript/functions.shiki.json").to_str().unwrap(), dir.join("javascript/functions.js").to_str().unwrap()
    )]).output().unwrap();
    ...
    for (pos, color) in c.iter().enumerate() {
        if !color.is_empty() && color.to_lowercase() != c[pos].to_lowercase() {
            ...
            println!("  pos {:>3} ({:?}): engine={} shiki={}", pos, ch, c[pos], color);
        }
    }
}
